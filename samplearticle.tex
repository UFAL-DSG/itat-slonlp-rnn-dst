...
\begin{abstract}
This paper discusses models for dialogue state tracking \PB{by} using \PB{the} recurrent neural networks (RNN).
We present experiments on the standard dialogue state tracking (DST) dataset, DSTC2~\cite{henderson2014second}.
On the one hand, RNN models became \PB{the} state of the art \PB{models} in DST,
on the other hand, most state-of-the-art models are only turn-based and require dataset-specific preprocessing (e.g. DSTC2-specific) in order to achieve \PB{the} state-of-the-art results.
...
\PB{At} first, we briefly introduce dialogue state tracking.
...
A dialogue state tracker summarizes the dialogue history and maintains \PB{the probability} distribution over user's goal (see annotation in~Figure~\ref{fig:example}).
...
User's goals are expressed in a~formal language, typically represented as a dialogue act item (DAI) and the tracker updates probability for each item \PB{nevime co je to DAI - asi chceme referenci na sec2}. 
Since the dialogue state is a latent variable it is called a~hidden information state (HIS)~\cite{young2010hidden} and one needs to label \PB{the} conversation \PB{in order} to train a dialogue state tracker using supervised learning.
...
We also experiment with re-splitting \PB{opravdu pomlcka?} of the DSTC2 data because there are considerable differences between the standard train and test data sets~\cite{henderson2014second}.
Since training, development and test set data \st{is} \PB{are} distributed differently, the resulting performance difference between training and test data is rather high.
Based on our experiments, we conclude that DSTC2 might suggest too pessimistic view of the state-of-the-art methods in dialogue state tracking \PB{that may be} caused by the data distribution mismatch.
...
The DSTC2 is a standard dataset for DST, and most of \PB{the} state-of-the-art systems in DST have reported their performance on~this dataset~\cite{henderson2014second}. 
The full \st{dataset} \PB{data set} \st{is} \PB{has been} freely available since January 2014 and \PB{it} contains 1612 dialogues in the training set, 506 dialogues in the development set and 1117 dialogues in the test set.\footnote{Available online at~\url{http://camdial.org/~mh521/dstc/}.}
...
The~database and \st{a} \PB{the}~manually designed ontology \st{which} \PB{that} captures a~restaurant domain are \PB{both} distributed with the \st{dataset} \PB{data set}.

\section{Models}\label{sec:model}
Our models are all based on a RNN encoder~\cite{werbos1990backpropagation}. 
\PB{nasledujici veta ma divny slovosled} Similarly to RNN encoder of~\cite{zilka2015incremental}, the models update their hidden states $h$ after processing each word. 
...
We use a~Gated Recurrent Unit (GRU) cell~\cite{cho2014gru} as the update function instead of simple RNN cell because it does not suffer from vanishing gradient~\cite{TODO_vanishing_gradient} \PB{chybi bibtex}.
The model optimizes its parameters during \PB{the} training \PB{neni tohle zjevne?}.
...
The features make the data dense by abstracting the meaning from lexical \PB{the} level. 
...
The second model uses a decoder in order to predict values one after \st{each other} \PB{another} from the $h_{T}$ (see Section~\ref{sec:encdec}).
...
The models were implemented \PB{by} using TensorFlow~\cite{abaditensorflow} framework. 
By introducing encoder-decoder architecture, we aim to overcome the data sparsity problem and \PB{the} incorrect independence assumptions.
...
\label{sec:indep}
\st{The independent model predicts slots $food$, $area$ and $price range$ based on the last hidden state $h_{T}$ independently using three classifiers.}
\PB{The independent model consists of three models which predict $food$, $area$ and $price range$ based on the last hidden state $h_{T}$ independently}
\PB{The} independent slot prediction \st{using} \PB{that use} one classifier per each slot is straightforward to implement, but the model introduces an unrealistic assumption of uncorrelated slot properties.
In case of DSTC2 and the Cambridge restaurant domain, it is hard to believe that, \st{e.g.} \PB{i.e.}, the slots $area$ and $price range$ are not correlated.
...
To our knowledge, we are \PB{the} first who used this model for the task of slot predictions.
The model is successfully used in machine translation where it is able to handle long sequences with good accuracy \PB{reference}.
In DST, it captures correlation between the decoded slots easily~\cite{bahdanau2014neural}. 

We \st{use} \PB{employ} the encoder RNN cell \PB{that} captures the history of \st{a} \PB{the} dialogue which is represented as a~sequence of words from \PB{the} user and \PB{the} system.
The words are fed to the encoder as they appear in \PB{the} dialogue - turn by turn - where \PB{the} user and \PB{the} system responses switch regularly.
The encoder updates its internal state $h_T$ after each \PB{processed} word.
...
The decoder generates \PB{sequences of words of} arbitrary length \st{sequence of words} given the encoded state $h_T$ step by step.
In each step, an output word and a~new hidden state $h_{T+1}$ is generated.
The generation process is finished when a special token End of Sequence (EOS) is generated. \PB{By this mechanism we allow the model to terminate the output sequence.}
...
Secondly, the decoding time is \PB{asymptotically} quadratic in the length of the decoded sequences \st{, but} \PB{even though} our target sequences are \st{only} \PB{always} four tokens long.
...
\PB{kazda veta zacina 'we', to se mi moc nelibi}
...
We use 1-best Automatic Speech Recognition (ASR) transcriptions of conversation history \st{on} \PB{of} the input and measure \PB{the} joint slot accuracy.
We evaluate the models using the recommended measure accuracy \PB{reference} with schedule 2 which skips the first turns where the believe tracker does not track any values. 
...
We train predicting \PB{the} goal slot values for each turn.
...
We use early stopping with patience~\cite{prechelt1998early}, validating on the development set after each epoch and stopping if the three top models does not change for four epochs \PB{tohle mi btw prijde dost malo, uz jsem potkal modely, ktere se z lokalniho minima hrabaly desitky epoch}.
...
The model with independent label prediction is a strong baseline which was used, among others, in work of~\cite{zilka2015incremental} \PB{podle me je lepsi pouzit 'citet' nez 'cite' kdyz referujeme primo autora}.
...
We \st{wanted} \PB{intended} to investigate \st{how this influences} \PB{the influence of} the complexity of the task, \st{so} \PB{hence} we merged all DSTC2 data together and created splits of 80\%, 10\% and 10\% for the training, development and test sets.
...
\PB{Since} there are numerous systems which reported on the DSTC2 dataset, we discuss only the systems which use RNNs.
...
which reported near state-of-the art results on the~DSTC2 dataset and \st{was} \PB{introduced} the first incremental system which was able to update the dialogue state word-by-word with such accuracy.
In contrast to work of~\cite{zilka2015incremental} \PB{'citet'}, we use no abstraction of slot values. 
Instead, we add the additional features as described in Section~\ref{sec:model}.
The first system which used a neural network for dialogue state tracking~\cite{henderson2013deep} used a feed-forward network and more than 10 manually engineered features across different levels of abstraction of the user input, including \PB{the} outputs of the spoken language understanding component (SLU).
...
The system of Henderson et al.~\cite{henderson2014word} \PB{'citet' misto 'cite'} achieves \PB{the} state-of-the-art results and, similarly to our system, it predicts the dialogue state from words by employing a RNN.
On the other hand, their system heavily relies on the user input abstraction.
Another dialogue state tracker with LSTM was used in \PB{the} reinforcement setting but the authors also used information from the SLU pipeline~\cite{lee2016dialog}.

An interesting approach is presented in the work of Vodolán et al.~\cite{vodolan2015hybrid} \PB{citet}, who combine a rule-based and a machine learning based approach.
The handcrafted features are fed to an LSTM-based RNN which performs a dialog-state update.
However, unlike our work, their system requires SLU \st{parses} \PB{parsers?} on the input.
...
However, the reported end-to-end model was evaluated only on artificial dataset and cannot be compared to DSTC2 \st{dataset} \PB{data set} directly.
...
To our knowledge, we are the first to use an encoder-decoder model for the dialogue state tracking task, and we encourage others to do so \PB{tohle je hodne neformalni} because it is competitive with the standard RNN model.\footnote{The presented experiments are published at \url{https://github.com/oplatek/e2end/} under Apache license. Informal experiments were conducted during the Statistical Dialogue Systems course at Charles University (see \url{https://github.com/oplatek/sds-tracker}).}
The models are comparable to state-of-the-art \PB{models} \PB{TODO citace}.

\PB{Zase vsechny vety zacinaji 'we'}
We evaluated the models on DSTC2 \st{dataset} \PB{data set} containing  task-oriented dialogues in \PB{the} restaurant domain. 
...
We observed that \PB{the} dialogue state tracking on DSTC2 test set is notoriously hard and that the task becomes substantially easier if the data is reshuffled.

As future work, we plan to investigate \PB{the} influence of the introduced database features on models accuracy.
To our knowledge there is no \st{dataset} \PB{data set} which can be used for evaluating incremental dialogue state trackers, so it would be beneficial to collect \st{annotation on word level} \PB{the word-level annotations} so one can evaluate incremental DST models.

\PB{opravdu tady chceme newpage? plytvani mistem...}
\newpage
