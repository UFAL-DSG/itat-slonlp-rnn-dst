...
Josef Válek \and Ondřej Plátek}

\institute{Charles University in Prague,\\
\email{oplatek@ufal.mff.cuni.cz},\\ WWW home page:
\texttt{http://ufal.mff.cuni.cz/ondrej-platek}}

\maketitle              % typeset the title of the contribution

\begin{abstract}
This paper presents three models for dialog state tracking task using recurrent neural networks (RNN).
We present experiments on standard dialog state tracking (DST) data set DSTC2 \cite{todo}.
We experiment with RNN models which one of them became state of the art in DST on DSTC2, but we focus on comparing the models rather than fine tune the preprocessing, which is typically used to establish new state-of-the-art.
