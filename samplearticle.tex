...
\begin{abstract}
This paper discuss alternatives for dialog state tracking using recurrent neural networks (RNN).
We present experiment on standard dialog state tracking (DST) dataset DSTC2 \cite{todo}.
RNN models became state of the art in DST. 
On the other hand most state-of-the-art models are only turn-based and some require preprocessing tailored for DSTC2. 
We implemented three architectures which can be used in incremental settings and requires almost no preprocessing.
We compare them to the benchmarks on DSTC2 and discuss their properties.
\end{abstract}
%
