...
on the other hand, most state-of-the-art \OD{DST} models are only turn-based and require dataset-specific preprocessing (e.g. DSTC2-specific) in order to achieve such results.
We implemented two architectures which can be used in incremental settings\OD{in an incremental setting} and require almost no preprocessing.
...
Such agents play \OD{the} role of \OD{a} domain expert on\OD{in a} narrow domain\OD{,} and users ask for information through conversation in natural language (see \OD{the example} system and user responses in~Figure~\ref{fig:example}).
A dialogue state tracker summarizes the dialogue history and maintains \OD{a} probability distribution over \OD{the (possible)} user's goal\OD{s} (see annotation in~Figure~\ref{fig:example}).
Dialogue agents as introduced in~\cite{young2010hidden} decide about \OD{a/the} next action based on the dialogue state distribution from\OD{given by/obtained from} the tracker.
User's goals are expressed in a~formal language, typically represented as a dialogue act item (DAI)\OD{DAIs? viz též komentář k obrázku} (see Section~\ref{sec:dst}) and the tracker updates probability for each item.
Since the dialogue state is a latent variable\OD{,} it is called a~hidden information state (HIS)~\cite{young2010hidden} and one needs to label the~conversations in order to train a dialogue state tracker using supervised learning.\OD{vynechal bych zmínku o HIS (citaci klidně nech), přijde mi matoucí}
...
\begin{figure}
   \dots \\
    ...
    \dots
    \caption{Example of golden annotation of Dialogue Act Item (DAI). The dialogue act item comprises from act type (all examples have type {\it inform}) and slots ({\it food, area, pricerange}) and their values (e.g. {\it Indian, west, None}).\OD{proč je tohle ``DAI'' a ne celý stav/DA?}}
\vspace{-0.70em}
\label{fig:example}
\end{figure}

\OD{tady ten text moc nenavazuje. já bych možná začal celý článek "Dialogue state tracking is..." a až sem pak hodil "This paper compares..." (bez tý věty "First, we briefly introduce...")}
...
Since \OD{the} training, development and test set data are distributed differently, the resulting performance difference between training and test data is rather high.
Based on our experiments, we conclude that DSTC2 might suggest \OD{a} too pessimistic view of the state-of-the-art methods in dialogue state tracking caused by the data distribution mismatch.


\section{Dialogue state tracking on DSTC2 dataset}\label{sec:dst}
Dialogue state trackers maintain their believes\OD{beliefs} about users\OD{'} goals by updating probabilities of dialogue history representations.
...
The models update their hidden states $h$ after processing each word similarly to \OD{the} RNN encoder of~\citet{zilka2015incremental}.
The encoder takes as inputs the previous state $h_{t-1}$\OD{,} representing history for first $t-1$ words\OD{,} and features $X_t$ for the current word $w_t$. 
It outputs \OD{the current} state $h_t$ representing the whole dialogue history up to current word.
We use a~Gated Recurrent Unit (GRU) cell~\cite{cho2014gru} as the update function instead of \OD{a} simple RNN cell because it does not suffer from \OD{the} vanishing gradient\OD{ problem}~\cite{hochreiter2001gradient}.
...
\begin{itemize}
    ...
    \item and also indicators describing whether the word is part of some\OD{a} named entity representing a value from the database.
\end{itemize}
Since \OD{the} DSTC2 database is a simple table with six columns, we introduce six binary features firing if the word is a substring of named entity at\OD{from the} given column.
...
Our model variants differ only in the way they predict {\it goal} labels, i.e.\OD{,} $food$, $area$ and $price range$ from the RNN's last encoded state.\footnote{Accuracy measure with schedule 2 on slot $food$, $area$ and $price range$ about which users can inform the system is a featured metric for DSTC2 challenge~\cite{henderson2014second}.} 
...
We also experimented with \OD{a} single classifier which predicts the labels jointly (see Figure~1),\OD{to má být figure 2? a nedal by se vyhodit a popisek zakomponovat do Fig.~\ref{fig:encind}?} but it suffers from data sparsity of the predicted tuples, so we focused only on the independent label prediction and encoder-decoder models.\OD{a vůbec celou tuhle větu bys mohl vynechat nebo přehodit do 3.1}

The models were implemented using \OD{the} TensorFlow~\cite{abaditensorflow} framework. 
By introducing \OD{an/the} encoder-decoder architecture, we aim to overcome the data sparsity problem and the~incorrect independence assumptions.\OD{nepatří tohle za (see Section 3.2)? tady mi to přijde trochu vytržený z kontextu. navic to “incorrect independence” vysvětluješ až v 3.1, tak možná to patří až do sekce 3.2?}
...
The~independent slot prediction that use\OD{s? / using} one classifier per \ODdel{each} slot is straightforward to implement, but the model introduces an unrealistic assumption of uncorrelated slot properties.
...
\caption{The RNN encodes the word history into dialogue state $h_T$ and predicts slot values independently.}\OD{odkaž na obrázek z textu}
\end{center}
\vspace{-0.80em}
\label{fig:encind}
\end{figure}
...
\label{sec:encdec}
The encoder-decoder model with attention~\cite{bahdanau2014neural} is the most sophisticated model\OD{to zní dost vznešeně, když porovnáváš dva} that we used for slot predictions.
...
\caption{Encoder decoder with attention predicts goals.}\OD{odkaž na obrázek z textu}
\label{fig:encdec}
\end{figure}
